...
const Header: React.FC<HeaderProps> = ({ currentUser, onLogoClick, onLoginClick, onSignUpClick, onLogoutClick, onNavigate }) => {
  return (
    <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg shadow-sm sticky top-0 z-50 border-b border-slate-200/70 dark:border-slate-800/70">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          ...
          >
            ...
            <span className="text-2xl font-bold text-gray-800 dark:text-slate-100 tracking-tight">RideLink</span>
          </div>
            ...
            {currentUser ? (
              <div className="flex items-center space-x-6">
                ...
                <div className="flex items-center gap-4 pl-2 border-l border-slate-200 dark:border-slate-700">
                   ...
                   <div 
                      ...
                      className="flex items-center gap-2 cursor-pointer p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"
                   >
                      <img src={currentUser.avatarUrl} alt={currentUser.name} className="w-8 h-8 rounded-full border-2 border-indigo-200" />
                      <span className="font-semibold text-gray-700 dark:text-slate-200">{currentUser.name}</span>
                   </div>
                   <button 
                      onClick={onLogoutClick}
                      className="bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-200 font-semibold px-4 py-2 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-all text-sm"
                    >
                    Log Out
                  </button>
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button 
                  ...
                  className="bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200 font-semibold px-4 py-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-all text-sm"
                >
                  Sign Up
                </button>
                <button 
          ...
          <button className="md:hidden text-gray-600 dark:text-slate-300">
