...
import { User, AISummary, Sentiment } from '../types';
import { summarizeReviews } from '../services/geminiService';
import { ShieldCheckIcon, StarIcon, SparklesIcon, UserCircleIcon, ChatBubbleIcon, type IconProps } from './icons/Icons';
...
const UserProfile: React.FC<UserProfileProps> = ({ user, currentUser, allUsers, onViewProfile }) => {
    ...
    <>
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">{isOwnProfile ? "My Profile" : `${user.name.split(' ')[0]}'s Profile`}</h1>
        </div>
                  ...
                  <div className="flex-1">
                      <div className="flex justify-between items-center mb-2">
                          <h4 
                            ...
                            title="This summary is generated by AI to give you a quick overview of the user's reputation."
                          >
                      ...
                      <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-md min-h-[120px] relative border border-slate-200 dark:border-slate-700">
                          {isLoadingSummary ? (
                          ...
                          <>
                              <p className="text-gray-700 dark:text-slate-300 text-sm leading-relaxed whitespace-pre-wrap">{aiSummary?.summary}</p>
              ...
              {!isOwnProfile && (
                  <div className="mt-6 border-t border-slate-200 dark:border-slate-700 pt-5">
                      <button
                          ...
                          className="w-full flex items-center justify-center gap-2 bg-white dark:bg-slate-700 border-2 border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-200 font-bold py-3 px-6 rounded-lg transition-all text-lg hover:bg-gray-100 dark:hover:bg-slate-600"
                      >
                      ...
                      <div key={review.id} className="flex items-start gap-4 border-b border-slate-200 dark:border-slate-700 pb-6 last:border-b-0 last:pb-0">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-center mb-1">
                            <p
                              onClick={() => rater && onViewProfile(rater)}
                              className="font-bold text-slate-800 dark:text-slate-100 cursor-pointer hover:underline"
                            >
                                {rater?.name}
                            </p>
      ...
      {isChatOpen && (
        <ChatModal
          ...
          rideId={`profile-chat-${user.id}-${currentUser.id}`}
        />
      )}
    </>
  );
