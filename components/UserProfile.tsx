        ...
        <aside className="lg:col-span-1">
          <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200 sticky top-28">
            <div className="flex flex-col items-center text-center">
              ...
              {user.isVerified && (
                <div className="flex items-center gap-1.5 text-sm text-green-700 font-semibold bg-green-100 px-3 py-1 rounded-full mt-3">
                  <ShieldCheckIcon className="w-4 h-4" />
                  <span>{user.verificationType} Verified</span>
                </div>
              )}
            </div>
            <div className="border-t border-slate-200 mt-6 pt-4 space-y-3 text-sm">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200">
            ...
            <div className="flex flex-col md:flex-row items-start gap-6">
                {/* Trust Score */}
                <div className="flex-shrink-0 w-full md:w-40 text-center bg-amber-50 border border-amber-200 rounded-lg p-4">
                    <p className="text-5xl font-extrabold text-amber-600">{user.trustScore.toFixed(1)}</p>
                    <p className="font-bold text-amber-800">Trust Score</p>
                    <div className="flex items-center justify-center mt-1">
                      {[...Array(5)].map((_, i) => (
                        <StarIcon key={i} className={`w-5 h-5 ${i < Math.round(user.trustScore) ? 'text-amber-400' : 'text-slate-300'}`} />
                      ))}
                    </div>
                </div>
                {/* AI Insights */}
            ...
            {user.reviews.length > 0 ? (
              <div className="space-y-6">
                {user.reviews.map(review => {
                  ...
                  return (
                    <div key={review.id} className="flex items-start gap-4 border-b border-slate-200 pb-6 last:border-b-0 last:pb-0">
                      <img src={rater?.avatarUrl} alt={rater?.name} className="w-12 h-12 rounded-full" />
                      <div className="flex-1">
                        <div className="flex justify-between items-center mb-1">
                          <p className="font-bold text-slate-800">{rater?.name}</p>
              ...
              <p className="text-slate-500 text-center py-4">You have not received any reviews yet.</p>
            )}
          </div>
        </div>
      </div>
